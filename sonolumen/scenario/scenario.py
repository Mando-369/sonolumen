...
composes a v1 `SimulationConfig`, calls `sonolumen.run()`, applies each
observer to the resulting trace, and returns a `ScenarioResult`.

Architectural rule: the v1 physics core (`sonolumen/runner.py`,
`sonolumen/bubble_dynamics.py`, ...) is read-only from here. This file
...
from sonolumen.scenario.field import combine_drives, standing_wave_factor
from sonolumen.scenario.io import (
...
class Scenario:
    ...
    def regime_classify(self, scenario_result: Any = None) -> str:
        ...
        if scenario_result is not None:
            try:
                from sonolumen.suggestions.regime import classify
    ...
    def run(self) -> ScenarioResult:
        ...
        from sonolumen.runner import run as v1_run
        ...
        metadata = {
            ...
            "sonolumen_version": _sonolumen_version(),
    ...
    def _compose_simulation_config(self) -> SimulationConfig:
        ...
        bit-identical to v1.
        """
        from sonolumen.liquids import liquid_with_corrections
        seed = self._effective_seed()
        if seed is None:
            raise ValueError(
...
def _check_minnaert_match(s: Scenario) -> list[Warning]:
    from sonolumen.bubble_dynamics import minnaert_frequency
...
def _check_wall_erosion_unknown(s: Scenario) -> list[Warning]:
    return [Warning(
        ...
        actionable_fix=(
            "build §13 (`sonolumen/material/`) to populate wall-load and "
...
def _sonolumen_version() -> str:
    try:
        from sonolumen._version import __version__
        return __version__
    except Exception:
        return "unknown"
...
def _apply_material_post_processing(scenario: Scenario, result: ScenarioResult) -> None:
    ...
    try:
        ...
        from sonolumen.material.materials import catalog as _materials_catalog
    except Exception:                                                    # noqa: BLE001
    ...
    from sonolumen.scenario.observers import StressProbeObserver
    ...
    try:
        from sonolumen.scenario.scenario import _chamber_volume
...
def _apply_suggestions_post_processing(scenario: Scenario, result: ScenarioResult) -> None:
    ...
    *not* run here — the user calls `sonolumen.suggestions.
    suggest_next_experiment(scenario, result, goal)` explicitly.
    """
    try:
        from sonolumen.suggestions.engine import SuggestionsEngine
    except Exception:                                                    # noqa: BLE001
