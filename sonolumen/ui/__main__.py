"""`python -m sonolumen.ui` entry point.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="sonolumen Dash UI")
    ...
    if args.debug:
        print(f"sonolumen UI (dev server) starting on http://{args.host}:{args.port}")
    ...
    app = create_app()
    print(f"sonolumen UI starting on http://{args.host}:{args.port}")
    print("(Ctrl-C to stop)")
    waitress_serve(app.server, host=args.host, port=args.port,
                    threads=4, ident="sonolumen")
