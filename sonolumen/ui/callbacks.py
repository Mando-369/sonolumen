...
from sonolumen.config import AcousticDrive, BubbleSeed
from sonolumen.liquids import preset as _liquid_preset
...
from sonolumen.ui import figures
from sonolumen.ui.audio import hydrophone_to_wav, wav_to_data_uri
from sonolumen.ui.state import (
    result_to_store,
    scenario_from_store,
    scenario_to_store,
)
...
def apply_controls(
    ...
    *,
    ...
    if (chamber_geometry is not None or chamber_radius_cm is not None
        ...
        if chamber_wall:
            try:
                from sonolumen.material.materials import get as _wall_get
...
def _blake_threshold_atm(R0_m: float, p_inf_atm: float = 1.0,
    ...
    UI status chip; not a substitute for `sonolumen.seed.blake_threshold`.
    """
    import math
...
def autodesign_refine(target_T_kK: float, must_be_stable: bool,
    ...
    from sonolumen.suggestions import DesignTarget, refine_design
    s = scenario_from_store(scenario_payload)
    if s is None:
...
def render_liquid_properties_card(scenario: Any) -> Any:
    ...
    try:
        from sonolumen.liquids import corrected_sound_speed_for
...
def render_bubble_properties_card(scenario: Any) -> Any:
    ...
    # Blake threshold (sonolumen.seed.blake_threshold) — pressure above
...
def _animation_time_readout(scenario_payload: Optional[str], frame_idx: int) -> str:
    ...
    from sonolumen.ui.style import ANIMATION_FRAME_BUDGET
    s = scenario_from_store(scenario_payload)
...
def _empty(title: str) -> go.Figure:
    from sonolumen.ui.style import PLOT_TEMPLATE
    fig = go.Figure()
...
def register_callbacks(app: Any) -> None:
    ...
    def _export_notebook(n_clicks, notebook):                           # noqa: ANN001
        ...
        return dict(content=csv_text, filename="sonolumen_notebook.csv")
    ...
    def _save(n_clicks, scenario_payload):                              # noqa: ANN001
        ...
        return dict(content=scenario_payload or "{}",
                    filename="sonolumen_scenario.json")
