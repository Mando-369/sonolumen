...
from cavplasma._constants import C_LIGHT, K_B
from cavplasma.bubble_dynamics import (
    minnaert_frequency,
    rayleigh_collapse_time,
    total_gas_molecules,
)
from cavplasma.config import SimulationConfig
from cavplasma.em_emission import emit_spectrum, photon_yield
from cavplasma.plasma import plasma_diagnostics, saha_trace
...
class SimulationResult:
    """Structured output of a single cavplasma run.
...
def run(cfg: SimulationConfig) -> SimulationResult:
    """Execute a full cavplasma simulation and return a SimulationResult.
    ...
    if output.detectors:
        from cavplasma.detectors import (
...
def _flash_FWHM_ns(
    ...
    """
    from cavplasma._constants import SIGMA_SB
    if len(t) < 3:
        return None
