...
Three presets ship with v2.0:
  ...
  * `sbsl_canonical()` — §9.3 SBSL (numerically identical to v1
    `cavplasma.presets.sbsl_canonical()`; passes the §12.10 #2 acceptance
    in <5 s with T_peak in band).
  * `pistol_shrimp_event()` — §9.8 pistol-shrimp impulsive event
    (numerically identical to v1 `cavplasma.presets.pistol_shrimp()`;
...
def sbsl_canonical() -> Scenario:
    ...
    Numerically identical to v1 `cavplasma.presets.sbsl_canonical()`: same
...
def pistol_shrimp_event() -> Scenario:
    ...
    Numerically identical to v1 `cavplasma.presets.pistol_shrimp()`: starts
